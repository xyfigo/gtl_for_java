...
public class Slave extends SlaveProxy{
    ...
    TITree createTITree(Triangle triangle,String dataFile ) throws  IOException{
        ...
        final TITree<LabeledInterval> tiTree=this.tree;
    ...
    boolean insert(List<LabeledInterval> timeline) {
        for(LabeledInterval i: timeline)
            tree.insert(i);
        return true;
    }


    boolean delete(List<LabeledInterval> timeline) {
        for(LabeledInterval i: timeline)
            tree.delete(i);
        return true;
    }
    ...
    List<Pair<Long, Double>> executeSimilarityQuery(Timeline t, Envelope e) {
        List<LabeledInterval> lr= tree.regionQuery(new RegionShape(e));
     ...
     * @return
     */
    List<Pair<Long, Double>> calculateSimilarity(Timeline t, List<LabeledInterval> b){
        JavaSparkContext sparkContext =JavaSparkContext.fromSparkContext(spark.sparkContext());
        JavaRDD<LabeledInterval>  aRDD=sparkContext.parallelize(t.getLabeledIntervals());
        JavaRDD<LabeledInterval>  bRDD=sparkContext.parallelize(b);
        return bRDD.cartesian(aRDD)
                .filter(r->r._1().getLabel().equals(r._2().getLabel()))
//                .mapToPair(r-> new Tuple2<Long,Double>(
//                                        Long.valueOf(r._1.getParentID().longValue()),
//                                        Double.valueOf(r._1.intersection(r._2))))
                .mapToPair(new PairFunction<Tuple2<LabeledInterval,LabeledInterval>, Long, Double>() {
                    public Tuple2<Long,Double>  call (
                            Tuple2<LabeledInterval,LabeledInterval> r){
     ...
     * @return
     */
    List<Pair<Pair<Long,Long>, Double>> calculateSimilarity(List<LabeledInterval> a, List<LabeledInterval> b){
        JavaSparkContext sparkContext =JavaSparkContext.fromSparkContext(spark.sparkContext());

        JavaRDD<LabeledInterval>  aRDD=sparkContext.parallelize(a);
        JavaRDD<LabeledInterval>  bRDD=sparkContext.parallelize(b);
        return bRDD.cartesian(aRDD)
                .filter(r->r._1().getLabel().equals(r._2().getLabel()))
//                .mapToPair(r-> new Tuple2<Long,Double>(
//                                        Long.valueOf(r._1.getParentID().longValue()),
//                                        Double.valueOf(r._1.intersection(r._2))))
                .mapToPair(new PairFunction<Tuple2<LabeledInterval,LabeledInterval>, Pair<Long,Long>, Double>() {
