...
public class TDTree<T extends Interval> extends BaseTriangleTree<T> {
    ...
    public List<T> regionQuery(final RegionShape rs) {
        constructRDD();
        final TriangleEncoder te = triangleEncoder;
        final Envelope2D e2d = rs.getMBR().flap();
        JavaRDD<T> result = sparkRDD
                .filter(r->{
                    //if(r.getValue().longValue()!=-1L) return false;
                    Triangle t = te.parse(r.getKey());
...
//                    //2. remove the ln from the map, and add the leftNode info to map
//                    leafInfos.remove(ln.first());
//                    leafInfos.put(leftNode.first(),page);
//                    //3. write the right node to a new page,and add the rightNode info to map
//                    page.reset(-1);
...
//                }
//                else{//还需要继续分裂
//                    //1. remove the ln from the map, and add the leftNode info to map
//                    leafInfos.remove(ln.first());
//
...
//                        //记录空节点，不用写入外存
//                        leafInfos.put(leftNode.first(),CommonSuits.createIdentifier(-1L));
//                        leafInfos.put(rightNode.first(),page);
//                    }
...
//                        leafInfos.put(leftNode.first(),page);
//                        //记录空节点，不用写入外存
//                        leafInfos.put(rightNode.first(),CommonSuits.createIdentifier(-1L));
//                    }
//                    repeatFlag=true;
//                    if(splitTimes==100)
//                        break;
