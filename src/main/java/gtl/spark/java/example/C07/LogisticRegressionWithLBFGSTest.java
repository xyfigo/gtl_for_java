...
public class LogisticRegressionWithLBFGSTest {
    public static void main(String[] args) {
        ...
        JavaRDD<LabeledPoint>[] splits = data.randomSplit(new double[]{0.7, 0.3}, 1234L);
        JavaRDD<LabeledPoint> training = splits[0].cache();
        JavaRDD<LabeledPoint> test = splits[1];

        // Run training algorithm to build the model.
        LogisticRegressionModel model = new org.apache.spark.mllib.classification.LogisticRegressionWithLBFGS()
                .setNumClasses(10)
                .run(training.rdd());
