...
object IrisLinearRegression {
    ...
    fun linearRegression(
        ...
        val data = setosa
                .toJavaRDD()
                .map { r -> LabeledPoint(r.getDouble(0), Vectors.dense(r.getDouble(1))) }
        val model = LinearRegressionWithSGD.train(data.rdd(), 10, 0.1)
        println("setosa萼片长度与宽度的线性回归方程：")
        println("y=" + model.weights() + " * x + " + model.intercept())
        //构建真实数据与预测数据的RDD
        val realAndPredictValues = data.map { t -> Tuple2(t.label(), model.predict(t.features())) }
        //计算均方误差
        val MSE = realAndPredictValues.map { t -> Vectors.dense(Math.pow(t._1() - t._2(), 2.0)) }
        println("setosa萼片长度与宽度的线性回归的均方误差为：" + Statistics.colStats(MSE.rdd()).mean())
    }
